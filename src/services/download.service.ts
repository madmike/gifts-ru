...
export class DownloadService {
  ...
  constructor(
    ...
    this.categoryParser.on(ProductEvents.PRODUCT_CATEGORY, async (data) => {
      this.store.setProductCategory(data);
    });

    this.stocksParser.on(ProductEvents.STOCK, async (data) => {
      this.store.addProductStocks(data);
    });
  }
  ...
  async getCategories() {
    ...
    lastValueFrom(this.http.get(url, { responseType: 'stream' })).then((res) =>
      res.data.pipe(this.categoryParser),
    );
  ...
  async getProducts() {
    ...
    lastValueFrom(this.http.get(url, { responseType: 'stream' })).then((res) =>
      res.data.pipe(this.productParser),
    );

    return new Promise<void>((resolve) => {
      this.productParser.on(SaxEvents.FINISH, async () => {
        this.store.addProduct().then(() => {
  ...
  async getStocks() {
    ...
    lastValueFrom(this.http.get(url, { responseType: 'stream' })).then((res) =>
      res.data.pipe(this.stocksParser),
    );
  ...
  async getPhotos() {
    const products = Product.find({ photos: { $ne: null } });
    return this.photos.start(products);
  }
}
